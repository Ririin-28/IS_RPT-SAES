...
export default function EnglishFlashcards({
  ...
  const buildPhonemeHint = useCallback((phonemes: PhonemeFeedback[]) => {
    ...
    return `Focus on the ${formatted} sound${unique.length > 1 ? "s" : ""}.`;
  }, []);
  ...
  const buildTutorFeedback = useCallback((
    ...
    let base = "";
    if (!lowWords.length) {
      base = "Great job! You pronounced the words clearly.";
    } else if (lowWords.length <= 2 && list) {
      base = `What a great effort! You\'re very close. Let\'s try the words again: ${list}.`;
    } else if (list) {
      base = `Let\'s practice together. Repeat these words: ${list}.`;
    } else {
      const trimmed = targetText.trim();
      const safeTarget = trimmed || "that";
      base = `Let\'s practice together. Repeat after me: ${safeTarget}.`;
    }
  ...
  const stopListening = useCallback(() => {
    ...
    if (browserRecognitionRef.current) {
      try {
        (browserRecognitionRef.current as any).stop();
      } catch {
        // ignore
      }
                      ...
                      sessionScores.map((item) => {
                        ...
                        return (
                          <tr key={item.cardIndex} className="border-t border-gray-200">
                            ...
                            <td className="py-2 pr-3 font-base text-[#013300]">{getReadingSpeedLabel(item.readingSpeedWpm)}</td>
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-center">
            <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center sm:justify-center gap-3 w-full">
              ...
              <button
                onClick={handleStopSession}
                disabled={!canSaveAndExit}
                className={`inline-flex items-center justify-center gap-2 rounded-full px-7 py-3 text-sm font-medium text-white shadow-md shadow-gray-200 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-emerald-600 w-full sm:w-auto ${
                  canSaveAndExit
                    ? "bg-[#013300] hover:bg-green-800 active:scale-95"
                    : "cursor-not-allowed bg-[#013300]/50 text-white/80"
                }`}
              >
                <span className="h-2 w-2 rounded-full bg-white/70" /> Save &amp; Exit

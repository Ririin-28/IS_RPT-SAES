...
export default function TableList({
  ...
  const bodyMaxHeightClass = isFullScreen
    ...
    : 'lg:max-h-[55vh] md:max-h-[60vh] max-h-[65vh]';

  return (
    <div
      className={`w-full flex flex-col rounded-lg ${
        isFullScreen ? 'fixed inset-0 z-50 bg-white p-6' : 'relative min-h-0'
      }`}
    ...
    >
      ...
      {!hidePagination && data.length > 0 && (
        <div
          className={`mt-2 flex shrink-0 items-center justify-center gap-2 py-2 text-sm sm:text-base border-t border-gray-200 bg-white rounded-lg shadow-sm ${
        ...
        >
          {data.length <= effectivePageSize ? (
            <>
              ...
              <span className="text-gray-500 font-semibold px-3 py-1.5 rounded bg-gray-100 cursor-default">
                1
              </span>
              ...
              {(() => {
                ...
                return pages.map((num, i) =>
                  typeof num === "number" ? (
                    ...
                    <span key={i} className="px-2 text-gray-400 font-semibold select-none">
                      {num}
                    </span>
                  )
                );

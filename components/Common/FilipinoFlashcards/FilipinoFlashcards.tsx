...
/* ---------- Filipino Remedial Flashcards data ---------- */
...
export default function FilipinoFlashcards({
  students,
  performances,
  onSavePerformance,
  initialView,
  ...
  onExit,
  subject: propSubject,
  phonemicLevel: propPhonemicLevel,
  activityTitle: propActivityTitle,
}: FilipinoFlashcardsProps) {
  ...
  const headerLevelLabel = useMemo(() => {
    const subjectLabel = toDisplaySubject(propSubject ?? subjectParam, "Filipino");
    const levelLabel = (propPhonemicLevel ?? phonemicNameParam).trim();
    return levelLabel ? `${subjectLabel} ${levelLabel}` : subjectLabel;
  }, [phonemicNameParam, propPhonemicLevel, propSubject, subjectParam]);
  ...
  let subtitle = propSubject && propPhonemicLevel
    ? `${propSubject} • ${propPhonemicLevel}`
    : phonemicNameParam || subjectParam
      ? headerLevelLabel
      : "";
  let mainTitle = propActivityTitle || selectedStudent.name;

  if (!subtitle && !propActivityTitle) {
      // Fallback to parsing student name if props aren't provided (backward compatibility)
        ...
        <div className="w-full max-w-8xl mx-auto px-4 sm:px-6 lg:px-10 py-6 flex min-h-dvh flex-col gap-5">
          ...
          <div className="grid gap-4 lg:grid-cols-12">
            <div className="rounded-3xl border border-gray-300 bg-white shadow-md shadow-gray-200 p-6 flex flex-col gap-3 lg:col-span-3">
              <p className="text-3xl sm:text-4xl font-bold text-black">Total Average</p>

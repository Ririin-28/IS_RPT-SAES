...
export default function MasterTeacherCalendar() {
  ...
  const handleOpenSendModal = () => {
    if (!activities.length) {
      showToast("Add at least one activity to your calendar before sending it to the principal.", "info");
      return;
    }

    const unresolvedSubject = activities.some((activity) => !activity.subject);
    if (unresolvedSubject && allowedSubjects.length === 0) {
      showToast("Assign a subject to each activity before sending them to the principal.", "info");
      return;
    }
  ...
  const handleSendActivities = async () => {
    ...
    try {
      ...
      showToast(error instanceof Error ? error.message : "Unable to send activities to the principal right now.", "error");
    } finally {
      setSending(false);
    }
  };

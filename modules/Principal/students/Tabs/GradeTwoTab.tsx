...
export default function GradeTwoTab({ students, setStudents, searchTerm }: GradeTwoTabProps) {
  const [showDetailModal, setShowDetailModal] = useState(false);
  const [selectedStudent, setSelectedStudent] = useState<any>(null);
  const [filter, setFilter] = useState({ section: "All Sections" });

  const gradeTwoStudents = students.filter(student => 
    student.grade === 2 || student.grade === "2"
  );

  const filteredStudents = gradeTwoStudents.filter((student) => {

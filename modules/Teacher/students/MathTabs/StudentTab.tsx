...
export default function StudentTab({ students, setStudents, searchTerm }: StudentTabProps) {
  ...
  const sortedStudents = useMemo(() => {
    ...
    const getStudentPhonemicValue = (student: any): string =>
      String(student?.mathProficiency ?? student?.math ?? "").trim();
  ...
  return (
    <div>
      ...
      <StudentDetailModal
        ...
        reportHref={
          selectedStudent
            ? `/Teacher/report/math/students/${encodeURIComponent(String(selectedStudent.studentId ?? selectedStudent.id ?? ""))}`
            : undefined
        }
      ...
      <TableList
        ...
        data={sortedStudents.map((student, idx) => ({
          ...
          phonemic: student.mathProficiency ?? student.math ?? "",
          no: idx + 1,
        }))}
        actions={(row: any) => (

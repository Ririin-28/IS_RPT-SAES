...
export default function StudentTab({ students, setStudents, searchTerm }: StudentTabProps) {
  ...
  const sortedStudents = useMemo(() => {
    ...
    const getStudentPhonemicValue = (student: any): string =>
      String(student?.englishPhonemic ?? student?.english ?? "").trim();
  ...
  const handleExport = () => {
    ...
    const filename = `Teacher_English_Students_${timestamp}.xlsx`;
  ...
  const handlePlayClick = (student: any) => {
    const run = async () => {
      ...
      if (!studentId) {
        setPlayErrorMessage("Student ID is missing.");
        return;
      }

      const phonemicLevel = String(student?.englishPhonemic ?? student?.english ?? "").trim();
      if (!phonemicLevel) {
        setPlayErrorMessage("Student phonemic level is missing.");
        return;
      }
      ...
      try {
        ...
        if ("error" in result) {
          setPlayErrorMessage(result.error);
          return;
        }
        ...
        setPlayErrorMessage("Unable to start remedial session.");
      } finally {
  ...
  return (
    <div>
      ...
      <StudentDetailModal
        ...
        reportHref={
          selectedStudent
            ? `/Teacher/report/english/students/${encodeURIComponent(String(selectedStudent.studentId ?? selectedStudent.id ?? ""))}`
            : undefined
        }
      ...
      <MessageModal
        isOpen={Boolean(playErrorMessage)}

...
export default function ITAdminProfile() {
  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      ...
      try {
        ...
        const response = await fetch("/api/it_admin/profile", { cache: "no-store" });
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await fetch("/api/it_admin/profile", requestInit);
      const payload = await response.json().catch(() => null);

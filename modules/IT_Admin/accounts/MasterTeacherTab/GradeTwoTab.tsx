...
export default function TeacherGradeTwoTab({ teachers, setTeachers, searchTerm }: GradeTwoTabProps) {
  ...
  const handleMenuAction = useCallback((action: AccountActionKey) => {
    ...
    console.log(`[Master Teacher Grade 2] Action triggered: ${action}`);
  }, []);
      ...
      try {
        ...
        const newTeachers = jsonData.map((row: any, index: number) => {
          ...
          return {
            ...
            grade: 2, // Force grade 2 for Grade Two tab
            section: row['SECTION'] || '',
          };
        });
  ...
  const filteredTeachers = useMemo(() => gradeTwoTeachers.filter((teacher) => {
  ...
  }), [gradeTwoTeachers, filter.section, searchTerm]);

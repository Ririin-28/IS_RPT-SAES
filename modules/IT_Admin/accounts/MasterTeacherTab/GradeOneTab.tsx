...
export default function TeacherGradeOneTab({ teachers, setTeachers, searchTerm }: GradeOneTabProps) {
  ...
  const handleMenuAction = useCallback((action: AccountActionKey) => {
    ...
    console.log(`[Master Teacher Grade 1] Action triggered: ${action}`);
  }, []);
      ...
      try {
        ...
        const newTeachers = jsonData.map((row: any, index: number) => {
          ...
          return {
            ...
            grade: 1, // Force grade 1 for Grade One tab
            section: row['SECTION'] || '',
          };
        });
  ...
  const filteredTeachers = useMemo(() => GradeOneTeachers.filter((teacher) => {
  ...
  }), [GradeOneTeachers, filter.section, searchTerm]);

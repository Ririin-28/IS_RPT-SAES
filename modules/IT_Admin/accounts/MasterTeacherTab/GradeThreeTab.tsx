...
export default function TeacherGradeThreeTab({ teachers, setTeachers, searchTerm }: GradeThreeTabProps) {
  ...
  const handleMenuAction = useCallback((action: AccountActionKey) => {
    ...
    console.log(`[Master Teacher Grade 3] Action triggered: ${action}`);
  }, []);
      ...
      try {
        ...
        const newTeachers = jsonData.map((row: any, index: number) => {
          ...
          return {
            ...
            grade: 3, // Force grade 3 for Grade Three tab
            section: row['SECTION'] || '',
          };
        });
  ...
  const gradeThreeTeachers = teachers.filter(teacher => 
    teacher.grade === 3 || teacher.grade === "3"
  );

  const filteredTeachers = useMemo(() => gradeThreeTeachers.filter((teacher) => {
  ...
  }), [gradeThreeTeachers, filter.section, searchTerm]);
    ...
    <div>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-4">
        <p className="text-gray-600 text-md font-medium">
          Total: {gradeThreeTeachers.length}
        </p>

...
export default function TeacherGradeFourTab({ teachers, setTeachers, searchTerm }: GradeFourTabProps) {
  ...
  const handleMenuAction = useCallback((action: AccountActionKey) => {
    ...
    console.log(`[Master Teacher Grade 4] Action triggered: ${action}`);
  }, []);
      ...
      try {
        ...
        const newTeachers = jsonData.map((row: any, index: number) => {
          ...
          return {
            ...
            grade: 4, // Force grade 4 for Grade Four tab
            section: row['SECTION'] || '',
          };
        });
  ...
  const filteredTeachers = useMemo(() => gradeFourTeachers.filter((teacher) => {
  ...
  }), [gradeFourTeachers, filter.section, searchTerm]);

...
export default function TeacherGradeFiveTab({ teachers, setTeachers, searchTerm }: GradeFiveTabProps) {
  ...
  const handleMenuAction = useCallback((action: AccountActionKey) => {
    ...
    console.log(`[Master Teacher Grade 5] Action triggered: ${action}`);
  }, []);
      ...
      try {
        ...
        const newTeachers = jsonData.map((row: any, index: number) => {
          ...
          return {
            ...
            grade: 5, // Force grade 5 for Grade Five tab
            section: row['SECTION'] || '',
          };
        });
  ...
  const filteredTeachers = useMemo(() => gradeFiveTeachers.filter((teacher) => {
  ...
  }), [gradeFiveTeachers, filter.section, searchTerm]);

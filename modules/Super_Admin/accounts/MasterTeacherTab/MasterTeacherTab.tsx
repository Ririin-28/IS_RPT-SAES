...
export default function MasterTeacherTab({ teachers, setTeachers, searchTerm, gradeFilter = "All Grades" }: MasterTeacherTabProps) {
  ...
  const handleSubmitAdd = useCallback(async (values: AddMasterTeacherFormValues) => {
    ...
    setSubmitError(null);
    try {
      const response = await fetch("/api/super_admin/accounts/masterteacher", {
        method: "POST",
    ...
    reader.onload = (event) => {
      void (async () => {
        try {
          ...
          const response = await fetch("/api/super_admin/accounts/masterteacher/upload", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
  ...
  const handleConfirmArchiveSelected = useCallback(async () => {
    ...
    setArchiveError(null);
    try {
      const response = await fetch("/api/super_admin/accounts/masterteacher/archive", {
        method: "POST",
        headers: { "Content-Type": "application/json" },

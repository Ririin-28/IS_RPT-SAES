...
export default function GradeFourTab({ students, setStudents, searchTerm }: GradeFourTabProps) {
  const [showDetailModal, setShowDetailModal] = useState(false);
  const [selectedStudent, setSelectedStudent] = useState<any>(null);
  const [filter, setFilter] = useState({ section: "All Sections" });

  const gradeFourStudents = students.filter(student => 
    student.grade === 4 || student.grade === "4"
  );

  const filteredStudents = gradeFourStudents.filter((student) => {
    ...
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-2">
        <p className="text-gray-600 mb-2 sm:mb-0">
          Showing {filteredStudents.length} of {gradeFourStudents.length} students
        </p>

...
interface MasterTeacherUserRow extends RowDataPacket {
  user_id: number;
  email: string | null;
...
function canonicalizeRole(role: string | null): string {
  ...
  const canonical = resolveCanonicalRole(normalized) || normalized;
  return canonical === "masterteacher" ? "master_teacher" : canonical;
}
...
export async function GET(): Promise<Response> {
  ...
  if (!session) {
    return new Response(JSON.stringify({ error: "Session expired" }), {
      ...
      headers: {
        ...
        "Set-Cookie": buildClearedMasterTeacherSessionCookie(),
      },
    });
  }
  ...
  try {
    const payload = await runWithConnection(async (connection) => {
      const [rows] = await connection.execute<MasterTeacherUserRow[]>(
      ...
      if (canonicalRole !== "master_teacher") {
        return null;
      }
    ...
    console.error("Master teacher session lookup failed", error);

...
export async function GET(request: NextRequest) {
  try {
    ...
    const effectiveGradeId = gradeId ?? null;
    const effectiveGradeLabel = gradeFilter ?? (effectiveGradeId ? `Grade ${effectiveGradeId}` : null);
    ...
    const activities: MasterTeacherActivity[] = rows
      .map((row) => {
        const activityDate = row.schedule_date ? String(row.schedule_date) : null;
        const rowGradeId = row.grade_id != null && Number.isFinite(Number(row.grade_id)) ? Number(row.grade_id) : null;
        const gradeLabel = rowGradeId ? `Grade ${rowGradeId}` : null;
        ...
        } satisfies MasterTeacherActivity;
      })
    ...
    console.error("Failed to load master teacher calendar activities", error);
    return NextResponse.json(
      { success: false, error: "Unable to load master teacher calendar activities." },
      { status: 500 },
    );
  }
}
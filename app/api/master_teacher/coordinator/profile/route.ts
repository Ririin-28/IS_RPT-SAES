  ...
  try {
    ...
    return NextResponse.json({
      ...
      metadata: {
        ...
        remedialGradeColumnsDetected: REMEDIAL_GRADE_COLUMN_CANDIDATES.filter((candidate) =>
          remedialColumns.has(candidate.column),
        ).map((candidate) => candidate.column),
        coordinatorTableSubjectColumnsDetected: COORDINATOR_TABLE_SUBJECT_CANDIDATES.filter((candidate) =>
          coordinatorColumns.has(candidate.column),
        ).map((candidate) => candidate.column),
      },
    });
  } catch (error) {

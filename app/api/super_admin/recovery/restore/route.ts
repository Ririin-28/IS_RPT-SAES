...
import { findRecoveryEntity, isArchiveBackedAccountEntity, modeFlagColumn, type RecoveryMode } from "@/lib/server/recovery-center";
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    if (isArchiveBackedAccountEntity(entityConfig.key)) {
      const origin = new URL(request.url).origin;
      const proxied = await fetch(`${origin}/api/super_admin/archive/restore`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          cookie: request.headers.get("cookie") ?? "",
        },
        body: JSON.stringify({ archiveIds: ids, reason, approvalNote }),
        cache: "no-store",
      });

      const proxiedBody = await proxied.json().catch(() => ({} as Record<string, unknown>));
      if (!proxied.ok) {
        const message =
          (proxiedBody as { error?: unknown })?.error && typeof (proxiedBody as { error?: unknown }).error === "string"
            ? String((proxiedBody as { error?: string }).error)
            : "Archive restore failed.";
        return NextResponse.json({ error: message }, { status: proxied.status });
      }

      const restoredEntries = Array.isArray((proxiedBody as { restored?: unknown[] }).restored)
        ? ((proxiedBody as { restored: unknown[] }).restored as Array<{ archiveId?: string | number; userId?: string | number }>)
        : [];
      const restoredIds = restoredEntries
        .map((entry) => entry.archiveId)
        .filter((value): value is string | number => value !== null && value !== undefined);

      return NextResponse.json({
        success: restoredIds.length > 0,

...
export async function POST(request: NextRequest) {
  ...
  if (archiveIds.length >= BULK_DUAL_AUTH_THRESHOLD && (!reason || !approvalNote)) {
    return NextResponse.json(
      { error: "Bulk restore requires both reason and approvalNote." },
      { status: 400 },
    );
  }
    ...
    const result = await runWithConnection(async (connection) => {
      ...
      await writeSecurityAuditLog(connection, {
        ...
        details: {
          ...
          restoredArchiveIds: restored.map((entry) => entry.archiveId),
          restoredUserIds: restored.map((entry) => entry.userId),
          errors,
          reason: reason || null,

...
export async function createItAdmin(input: CreateItAdminInput): Promise<CreateItAdminResult> {
  ...
  const result = await runWithConnection(async (connection) => {
    ...
    let superAdminColumns: Set<string> | null = null;
    try {
    ...
    try {
      superAdminColumns = await getColumnsForTable(connection, "super_admin");
    } catch {
      superAdminColumns = null;
    }
        ...
        const [userRoleRows] = await connection.query<RowDataPacket[]>(
          `SELECT u.role_id
           ...
           WHERE u.role_id IS NOT NULL
             AND LOWER(REPLACE(REPLACE(TRIM(r.role_name), '-', '_'), ' ', '_')) IN ('super_admin','admin','it_admin','itadmin')
           ORDER BY u.role_id
           LIMIT 1`,
        );
    ...
    try {
      ...
      if (superAdminColumns && superAdminColumns.size > 0) {
        const superAdminInsertColumns: string[] = [];
        const superAdminValues: any[] = [];

        if (superAdminColumns.has("user_id")) {
          superAdminInsertColumns.push("user_id");
          superAdminValues.push(userId);
        }
        if (adminId && superAdminColumns.has("super_admin_id")) {
          superAdminInsertColumns.push("super_admin_id");
          superAdminValues.push(adminId);
        }
        if (adminId && superAdminColumns.has("it_admin_id")) {
          superAdminInsertColumns.push("it_admin_id");
          superAdminValues.push(adminId);
        }
        if (adminId && superAdminColumns.has("admin_id")) {
          superAdminInsertColumns.push("admin_id");
          superAdminValues.push(adminId);
        }
        if (superAdminColumns.has("first_name")) {
          superAdminInsertColumns.push("first_name");
          superAdminValues.push(firstName);
        }
        if (superAdminColumns.has("middle_name")) {
          superAdminInsertColumns.push("middle_name");
          superAdminValues.push(middleName);
        }
        if (superAdminColumns.has("last_name")) {
          superAdminInsertColumns.push("last_name");
          superAdminValues.push(lastName);
        }
        if (superAdminColumns.has("suffix")) {
          superAdminInsertColumns.push("suffix");
          superAdminValues.push(suffix);
        }
        if (superAdminColumns.has("name")) {
          superAdminInsertColumns.push("name");
          superAdminValues.push(fullName);
        }
        if (superAdminColumns.has("email")) {
          superAdminInsertColumns.push("email");
          superAdminValues.push(email);
        }
        if (superAdminColumns.has("contact_number")) {
          superAdminInsertColumns.push("contact_number");
          superAdminValues.push(phoneNumber);
        }
        if (superAdminColumns.has("phone_number")) {
          superAdminInsertColumns.push("phone_number");
          superAdminValues.push(phoneNumber);
        }
        if (superAdminColumns.has("status")) {
          superAdminInsertColumns.push("status");
          superAdminValues.push("Active");
        }
        ...
        if (superAdminColumns.has("role_id") && adminRoleId !== null) {
          ...
          superAdminValues.push(adminRoleId);
        }
        if (superAdminColumns.has("created_at")) {
          superAdminInsertColumns.push("created_at");
          superAdminValues.push(now);
        }
        if (superAdminColumns.has("updated_at")) {
          superAdminInsertColumns.push("updated_at");
          superAdminValues.push(now);
        }

        if (superAdminInsertColumns.length > 0) {
          const superAdminColumnsSql = superAdminInsertColumns.map((column) => `\`${column}\``).join(", ");
          const superAdminPlaceholders = superAdminInsertColumns.map(() => "?").join(", ");
          await connection.query<ResultSetHeader>(
            `INSERT INTO super_admin (${superAdminColumnsSql}) VALUES (${superAdminPlaceholders})`,
            superAdminValues,
          );
        }
      }

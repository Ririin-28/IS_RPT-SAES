...
export async function POST(request: NextRequest) {
  try {
    ...
    const params: Array<number | string | null> = [subjectId, phonemicId, phonemicId, approvedScheduleId, ...studentIds];
    ...
    const [rows] = await query<RowDataPacket[]>(
      `
        SELECT
          ...
          EXISTS(
            SELECT 1
            FROM student_phonemic_history h
            WHERE h.student_id = s.student_id
              AND h.subject_id = ?
              AND (? IS NULL OR h.phonemic_id = ?)
            LIMIT 1
          ) AS has_phonemic
    ...
    const statusByStudent: Record<string, { completed: boolean }> = {};
    for (const studentId of studentIds) {
      statusByStudent[studentId] = { completed: false };
    }

    for (const row of rows) {
      const studentId = String(row.student_id);
      const hasPerformance = Boolean(row.has_performance);
      const hasPhonemic = Boolean(row.has_phonemic);
      const completed = hasPerformance && hasPhonemic;
      statusByStudent[studentId] = { completed };
    }

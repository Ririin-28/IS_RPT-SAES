...
import { canManagePrincipalCalendars } from "@/lib/server/emergency-access";
...
const buildRequesterName = (row: ApprovedRow): string | null => {
  ...
  return (suffix ? `${base} ${suffix}`.trim() : base) || null;
};
  ...
  try {
    return await runWithConnection(async (connection) => {
      ...
      if (!permission.allowed) {
        return NextResponse.json({ success: true, locked: true, activities: [] });
      }
      ...
      if (!idColumn || !dateColumn) {
        return NextResponse.json({ success: true, locked: false, activities: [] });
      }
      ...
      const addUserColumn = (column: string, alias: string) => {
        if (userColumns.has(column)) userSelectParts.push(`u.${column} AS ${alias}`);
      };
      ...
      if (hasArchiveColumn) whereParts.push("COALESCE(r.is_archived, 0) = 0");
      if (gradeId && gradeColumn) {
      ...
      const [rows] = await connection.query<ApprovedRow[]>(sql, params);
      ...
      for (const row of rows) {
        ...
        if (uniqueByDate.has(dateKey)) continue;
      ...
      return NextResponse.json({ success: true, locked: false, activities: Array.from(uniqueByDate.values()) });
    });
  } catch (error) {
    console.error("Failed to load emergency calendar activities", error);
